...
class EditEmailTemplateScreen extends StatefulWidget {
  ...
  const EditEmailTemplateScreen({super.key, required this.template});
...
class _EditEmailTemplateScreenState extends State<EditEmailTemplateScreen> {
  ...
  void initState() {
    ...
    subjectController.text = widget.template.subject ?? "";
    templController.text = widget.template.content ?? "";
    context.read<EmailCubit>().selectedEmailType = EmailType(type: widget.template.type!.capitalizeFirst(), value: widget.template.type!.toLowerCase());
  }
